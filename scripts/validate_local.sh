...
if [ -f "$APP_DIR/.action_token" ]; then
  ...
  HTTP_CODE="$(curl -s -o /dev/null -w '%{http_code}' "$BASE_URL/actions/reboot?token=$TOKEN" || true)"
  if [ "$HTTP_CODE" = "200" ]; then
    if curl -fsS "$BASE_URL/actions/reboot?token=$TOKEN" | grep -q "Action not allowed"; then
      pass "non-whitelisted action rejected"
    else
      fail "non-whitelisted action response unexpected"
    fi
  else
    fail "non-whitelisted action unexpected HTTP $HTTP_CODE"
  fi
